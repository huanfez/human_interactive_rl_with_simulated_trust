...
def sample_beta2_4_maxprob(interested_vals, posterior_mean, posterior_var, iteration_num):
    """
        Args:
    ...
    beta2_4_prob_dict = {beta2_4_val: posterior_distribution.pdf(np.array(beta2_4_val)) for beta2_4_val in interested_vals}

    max_beta2_4_prob = 0.0
    beta2_4_maxprob = interested_vals[0]
    for beta2_4 in beta2_4_prob_dict.keys():
        if beta2_4_prob_dict[beta2_4] > max_beta2_4_prob:
            max_beta2_4_prob = beta2_4_prob_dict[beta2_4]
            beta2_4_maxprob = beta2_4

    return [beta2_4_maxprob]
...
def get_interested_beta2_4(betas_mean, betas_var):
    ...
    beta2 = set([value for value in np.arange(betas_l[2], betas_u[2], 0.5) if -0.2 <= value <= 1.1])
    beta3 = set([value for value in np.arange(betas_l[3], betas_u[3], 0.5) if -0.2 <= value <= 1.1])
    beta4 = set([value for value in np.arange(betas_l[4], betas_u[4], 0.5) if -1.1 <= value <= 1.1])
        ...
        for state in model["state_actions_rewards"][beta].keys():
            for action in model["state_actions_rewards"][beta][state].keys():
                ...
                if next_state in model["acc_states"]:
                    model["state_actions_rewards"][beta][state][action][2] = 10.0 + np.matmul(np.array(beta[:2]), (feature1 + feature2) / 2.0) + beta[2]
...
if __name__ == '__main__':
    try:
        ...
        init_model = environment.get_prod_mdp_model(setlearn.crop_topLeft, setlearn.crop_bottomRight,
                                                    ...
                                                    setlearn.mdp_init_state, ltl_dfa.dfa1_dotFormat)
        # interested_betas = get_interested_beta2_4(setlearn.Beta0_itr, setlearn.Sigma0_itr)  # interested beta values
        interested_betas = [(-0.035362127101256746, 0.040000000000000036, -0.6353621271012564),
                            (-0.035362127101256746, 0.54, -0.1353621271012564),
                            (0.46463787289874325, 0.54, -0.1353621271012564)]
        ...
        agent_info = {"assumed_model": init_param_model["state_actions_rewards"], "actions": ["E", "S", "W", "N", "c"],
                      ...
                      "discount": 0.95, "step_size": 0.1, "epsilon": 0.1, "planning_steps": 20, 'random_seed': 35,
        ...
        # rewards_mat = reward_map(setlearn.beta_true3[2:5], 0, state_location_map, 5, 5)
        ...
        for iteration in range(1, episodes):
            # get prior of beta, and sample/obtain one beta with max probability from interested beta
            prior_beta2_4_mean, prior_beta2_4_var = setlearn.Beta0_itr[2:], setlearn.Sigma0_itr[2:, 2:]
            sampled_beta2_4 = sample_beta2_4_maxprob(interested_betas, prior_beta2_4_mean, prior_beta2_4_var, iteration)
            ...
            while len(route) < 10:
                # choose an action (randomly, but only one action can use)
            ...
            optimo_cell_path = [extract_mdp_state(loc_state, init_model["states_locations"])[1] for loc_state in route]
            print "epis:", iteration, " beta2_4:", sampled_beta2_4, "route:", route
            for beta2_4 in interested_betas:
                if beta2_4 not in sampled_beta2_4:
            ...
            # route_list = sampled_route_dict.values()
            # cell_path_array = state2cell_path_converter(route_list, state_location_map)  # convert state to cell
            # optimo_cell_path = cps2.optimal_cell_path_mixture(cell_path_array, state_features)
